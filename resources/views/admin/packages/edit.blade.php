<x-layouts.admin>
    <x-slot name="title">Edit Paket: {{ $package->name }}</x-slot>
        ...
        <form action="{{ route('admin.packages.update', $package->id) }}" method="POST">
            ...
            <div style="background: white; border-radius: 16px; border: 1px solid var(--border); padding: 24px; margin-bottom: 25px; box-shadow: 0 2px 8px rgba(120, 53, 15, 0.02);">
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <input type="text" id="selling_price" name="selling_price" value="{{ old('selling_price', $package->selling_price) }}" required
                           placeholder="95.000"
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <textarea name="description" rows="3"
                               ...
                               onfocus="this.style.borderColor='var(--brown-500)'" onblur="this.style.borderColor='var(--border)'">{{ old('description', $package->description) }}</textarea>
                    @error('description') <span style="color: #ef4444; font-size: 11px; margin-top: 4px; display: block;">{{ $message }}</span> @enderror
                </div>

                <div style="margin-bottom: 20px; display: flex; align-items: center; gap: 8px;">
                    <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $package->is_active ? '1' : '0') == '1' ? 'checked' : '' }}
                           style="width: 16px; height: 16px; accent-color: var(--brown-500); cursor: pointer;">
                    <label for="is_active" style="font-size: 13.5px; font-weight: 600; color: var(--text-main); cursor: pointer; user-select: none;">Aktifkan Paket</label>
                </div>
    ...
    <script>
        ...
        // Reload old inputs if exists (validation fails), otherwise load existing package items
        @if(old('items'))
