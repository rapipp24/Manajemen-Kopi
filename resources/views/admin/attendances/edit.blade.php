<x-layouts.admin>
    ...
    <div style="margin-bottom: 24px;">
        <h1 style="font-size: 20px; font-weight: 700; color: var(--text-main); margin: 0;">Edit Absensi Karyawan Gudang</h1>
        <p style="font-size: 13px; color: var(--text-muted); margin: 4px 0 0 0;">Perbarui data absensi yang telah dicatat.</p>
    </div>
    ...
    <div style="background: white; border-radius: 16px; border: 1px solid var(--border); padding: 28px; max-width: 520px; box-shadow: 0 2px 8px rgba(120, 53, 15, 0.02);">
        <form action="{{ route('admin.attendances.update', $attendance->id) }}" method="POST">
            ...
            <div style="margin-bottom: 20px;">
                <label style="display: block; font-size: 12.5px; font-weight: 700; color: var(--text-mid); margin-bottom: 6px;" for="warehouse_employee_id">Karyawan <span style="color:#dc2626;">*</span></label>
            ...
            <div style="margin-bottom: 24px;">
                ...
                <textarea name="note" id="note" rows="3"
                          ...
                          onfocus="this.style.borderColor='var(--brown-500)'" onblur="this.style.borderColor='var(--border)'">{{ old('note', $attendance->note) }}</textarea>
                @error('note')
                    <span style="color: #dc2626; font-size: 12px; margin-top: 4px; display: block;">{{ $message }}</span>
                @enderror
            </div>
